...
export async function generateMetadata(): Promise<Metadata> {
  ...
  const page = await getCmsPage("how-to-come");
  return buildPageMetadata("how-to-come", page?.theme?.seo);
}

export default async function HowToComePage() {
  // CMS에 보이는 블록이 있으면 CMS로 렌더, 없으면 기존 콘텐츠 폴백
  const page = await getCmsPage("how-to-come");
  if (page && page.blocks.length > 0) {
    return <CmsPageShell page={page} activePage="/how-to-come" />;
  }
  return <HowToComeFallback />;
}

function HowToComeFallback() {
  return (
    <>
