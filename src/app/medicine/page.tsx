...
export async function generateMetadata(): Promise<Metadata> {
  ...
  const page = await getCmsPage("medicine");
  return buildPageMetadata("medicine", page?.theme?.seo);
}
...
export default async function MedicinePage() {
  ...
  if (page && page.blocks.length > 0) {
    return <CmsPageShell page={page} activePage="/medicine" />;
  }
  return <MedicineFallback />;
}

function MedicineFallback() {
  return (
